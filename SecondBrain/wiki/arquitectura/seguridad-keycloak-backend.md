...
`reusable:alta` — el patrón completo (resource server + admin client + role converter desde `resource_access`) es portable prácticamente sin cambios, solo actualizando `CLIENT_ID`, nombre de realm y prefijos de propiedades. Es una de las piezas más maduras y consistentes de todo el backend actual.
